...
def actionSteps(current_set,b_left,max_time):
    # belief update
 time=0
 while time < max_time:
    Values=[]
    action=pickBestAction(b_left,current_set)
    print("The best action is:",action)
    observation = input("What do you hear? ")
    for next_state in states:
        Z=sensor(observation,action,next_state)
        value=0
        for initial_state in states:
        ...
        Values.append(Value)
    print("values",Values)
    b_left=Values[0]/(Values[0]+Values[1])
    print("current belief is ",b_left)
 time+=1
...
print(action_map)
actionSteps(current_set,0.5,20)
